            ...
            switch(wParam)
            {
                ...
                case Recovery_File :
                    ...
                    if(l==NULL)
                    {
                        ...
                        if(fp==NULL)
                        {
                             MessageBox(hwnd,"Le fichier est déjà vide", "", MB_OK | MB_ICONERROR ) ;
                        }
                        else
                        {
                             l=Recuperation_Contact() ;
               ...
               /*ADD*/
                ...
                case 6700 :
                    ...
                    if(TestInfo(hwnd))
                    {
                        ...
                        {
                           lpl->Info=c ;
                            if (MessageBox(hwnd, "Le numéro de contact a été modifié avec succés\n\nVoulez vous modifier le numéro d'un autre contact ?", "", MB_YESNO | MB_ICONQUESTION ) == IDYES)
                            {
                                DestroyAlll(hwnd) ;
                                SearchEdit(hwnd,2) ;
                            }
...
void SearchDel(HWND hwnd,int choix)
{
    ...
    if(choix==4)
    {
        ...
        Numt = CreateWindow(TEXT("Edit"),"Numéro : ",WS_VISIBLE | WS_CHILD | ES_READONLY | ES_NUMBER, 550, 130, 70, 25,hwnd,NULL,NULL,NULL) ;
